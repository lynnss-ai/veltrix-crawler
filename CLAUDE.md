...
5. 边采边入库(`persist_collected`,on-conflict upsert 判重,更新点赞/评论等统计)。阶段顺序:内容采集 → 作者画像补采 → **评论采集** → 直链补取(开「音频提取」时;刻意排在评论后、关窗前,保证下载时直链最新)→ 关窗放锁(需要窗口的阶段全部在一次开窗内完成)→ 素材下载(封面/头像/视频转音频,并发 10 路,不占窗口)→ **语音转写(「AI 文案提取」)** → **评论意向分析** → Obsidian 同步 → 落 `completed`。
