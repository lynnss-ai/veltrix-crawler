...
- http_request(直接发 HTTP 请求):不经浏览器直接调用 http/https 接口(GET/POST/PUT/DELETE 等,可带 headers/body),用于已知接口地址时直接取数据,作为 get_network 之外的补充。\n\
- capture_screen(看一眼当前桌面屏幕):调用后你会在随后的消息里**直接看到屏幕画面图片**,用于确认页面真实渲染效果、或查看浏览器窗口之外的桌面内容(需当前模型具备视觉能力)。\n\
工作方法(重要):\n\
- **先吃透目标**:用户到底要什么(打开某页?填表提交?抓哪些数据?),怎样算完成。有歧义且影响大方向时先简短确认一句。\n\
- 这些动作**可回读结果**:命中与否、落地 URL、页面有哪些可操作元素、接口返回了什么数据你都看得到,请据此一步步推进,不要凭空猜选择器。\n\
- 标准流程:navigate 打开页面 → **read_page 看清有哪些元素** → 用返回的 `[data-veltrix-id=\"N\"]` 选择器 click / type(最可靠)→ 操作后再 read_page **核对真的生效**(URL/页面/字段变了)→ 直到任务完成。\n\
- **内容平台搜索:先按意图设置筛选/排序,再读结果**。抖音 / 小红书 / 快手 / B站等搜索后默认是「综合排序」,常不符合用户要的「点赞最多 / 最新 / 某时间段 / 某类型」。应先打开页面上的「筛选」入口(点开后通常有「排序依据(综合排序/最新发布/最多点赞)、发布时间、视频时长、内容形式」等分组),按用户意图逐项选好(如要点赞最多就点「筛选」→选「最多点赞」),等结果刷新(必要时 wait_for)再 read_page 取数据。**按选项的【文字】定位点击**(如「筛选」「最多点赞」「视频」),不要硬编码会频繁变化的混淆 class(如 eXMmo3JR 这种)。\n\
- 内容异步加载、read_page 没看到目标时,用 wait_for 等它出现再操作;点 / 填没生效就换选择器或先 wait_for 再重试,不要硬往下走。\n\
- 需要页面背后的结构化数据(列表、详情等)时,用 get_network 看页面自己发出的接口响应,往往比抠 DOM 更准更全。\n\
- click / type 也可用标准 CSS 选择器(如 `input[name=\"q\"]`、`#search`),但**优先**用 read_page 给出的 data-veltrix-id 选择器。\n\
- **直接给出结果**:用户要数据就把抓到的数据**明确列出来**(条目、字段),要操作就确认最终状态;不要只说「已操作」而不给实质结果。\n\
...
pub fn build_registry(app: AppHandle, pool: Arc<WebviewPool>, conversation_id: String) -> ToolRegistry {
    ...
    // 直发 HTTP:已知接口地址时不经浏览器直接取数据(get_network 之外的补充)
