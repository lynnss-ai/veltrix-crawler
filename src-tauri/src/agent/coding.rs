...
//! ReAct 循环在 `commands::coding::send_coding_message`(便于逐步落库 + 推前端进度)。
...
- 全部做完后再用简洁中文总结「做了什么、运行结果如何」(此时不再调用工具)。\n\
...
/// 持久化由 commands::coding::send_coding_message 拦截本工具调用把 todos 写入会话(保持分层)。
...
pub async fn checkpoint(workspace: &Path, exec: &ExecConfig) {
    for cmd in [
        "git init -q",
        "git add -A",
        "git -c user.email=agent@veltrix.local -c user.name=veltrix commit -q -m \"veltrix checkpoint\" --allow-empty",
    ] {
